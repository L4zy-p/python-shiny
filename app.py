...
def style_plotly_chart(fig, yaxis_title):
    fig.update_layout(
        ...
        plot_bgcolor="#3e3e42",  # Background color of the plot area
        paper_bgcolor="#3e3e42",  # Background color of the entire canvas
...
# page title
ui.page_opts(window_title="Python shiny dashboard", fillable=False)
...
with ui.card(class_="card"):
    ui.card_header("Sales by City 2023")

    with ui.layout_sidebar():
        with ui.sidebar(bg="#29292e", open="open"):
            ui.input_selectize(
                "city",
                "Select a City:",
                [
        ...
        def sales_over_time_altair():
            ...
            chart = (
                ...
                .properties(title=alt.Title(f"Sales over Time -- {input.city()}", color="#fff"))
                .configure_axis(grid=False)
                .configure_title(font="Arial", color=FONT_COLOR)
                .configure(background="#3e3e42")
                .configure_view(fill="#3e3e42")
            )
...
with ui.layout_column_wrap(width=1 / 2):
    ...
    with ui.card(class_="card"):
        ui.card_header("Sales by Time of Day Heatmapc")
        ...
        def plot_sales_by_time():
            ...
            plt.gca().set_facecolor('#2e2e2e')  # Background color of the plotting area
